...
class TestTargetSqlMatchesTheSchema:
    ...
    def test_the_parser_reads_every_table_in_the_schema(self) -> None:
        """Guards the regex itself: the body pattern is non-greedy, so a
        table whose terminator it misses is silently folded into the previous
    ...
    def test_the_check_covers_the_unambiguous_targets(self) -> None:
        """A stripping or matching regex that stopped matching would leave
        nothing to check and still pass the assertion above."""
        assert len(_single_table_targets()) >= 15  # 18 of 35 targets when written
