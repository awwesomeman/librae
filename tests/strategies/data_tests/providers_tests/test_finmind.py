"""Tests for strategies.module.data.providers.finmind — the bare FinMind HTTP call."""
...
from strategies.module.data.providers.finmind import fetch
...
def test_fetch_returns_data_rows_on_success():
    with patch("strategies.module.data.providers.finmind.urllib.request.urlopen", return_value=_mock_response({"status": 200, "data": [{"a": 1}]})):
...
def test_fetch_raises_on_non_200_status():
    with patch("strategies.module.data.providers.finmind.urllib.request.urlopen", return_value=_mock_response({"status": 400, "msg": "paid tier only"})):
...
def test_fetch_includes_data_id_and_token_in_url():
    ...
    with patch("strategies.module.data.providers.finmind.urllib.request.urlopen", side_effect=_fake_urlopen):
