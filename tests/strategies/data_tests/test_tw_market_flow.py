"""Tests for strategies.data.tw_market_flow — market-wide margin/short
...
from strategies.data.tw_market_flow import (
...
def test_fetch_tw_margin_balance_history_uses_pseudo_symbol():
    ...
    with patch("strategies.data.tw_market_flow.get_factor", return_value=fake) as mock_get:
...
def test_margin_balance_fetcher_reads_today_balance():
    from strategies.data.tw_market_flow import _margin_balance_fetcher
    ...
    with patch("strategies.data.tw_market_flow.finmind_client.fetch", return_value=raw_rows):
...
def test_market_net_buy_fetcher_computes_buy_minus_sell():
    from strategies.data.tw_market_flow import _market_net_buy_fetcher
...
def test_empty_rows_return_empty_frame_with_correct_columns():
    ...
    fetch_fn = _margin_balance_fetcher("MarginPurchase")
    with patch("strategies.data.tw_market_flow.finmind_client.fetch", return_value=[]):
        result = fetch_fn("TWSE", datetime(2024, 1, 1, tzinfo=timezone.utc), datetime(2024, 1, 2, tzinfo=timezone.utc))
...
def test_attach_tw_market_flow_features_fills_missing_with_zero():
    from strategies.data.tw_market_flow import attach_tw_market_flow_features

    ohlcv = pd.DataFrame({"timestamp": pd.to_datetime(["2024-01-02T00:00:00Z"]), "close": [100.0]})
    empty = pd.DataFrame(columns=["timestamp", "value"])

    with patch("strategies.data.tw_market_flow.get_factor", return_value=empty):
