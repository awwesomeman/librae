...
def test_schema_defines_idempotent_runtime_event_key() -> None:
    ...
    assert "ON runtime_events(run_id, ts, event_type)" in sql
...
def test_write_runtime_event_upserts_same_event(mock_get_conn: MagicMock) -> None:
    ...
    write_runtime_event(
        ...
        detail={"active_orders": 2},
    )

    sql, values = cursor.execute.call_args.args
    assert "ON CONFLICT (run_id, ts, event_type) DO UPDATE" in sql
    assert values == (ts, "run-1", "state_recovered", '{"active_orders": 2}')


@patch("librae.db.timescale_writer.get_conn")
def test_write_runtime_event_allows_missing_detail(mock_get_conn: MagicMock) -> None:
    connection = MagicMock()
    connection.__enter__.return_value = connection
    cursor = connection.cursor.return_value
    mock_get_conn.return_value = connection
    ...
    assert values == (ts, "run-1", "state_recovered", None)
