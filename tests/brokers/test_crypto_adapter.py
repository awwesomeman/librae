...
@pytest.mark.parametrize(("digits", "expected_tick"), [(2, 0.01), (-1, 10.0)])
def test_available_symbols_converts_decimal_places_only_for_costs(
    readonly_adapter,
    mock_ccxt_exchange,
    digits,
    expected_tick,
    ...
    assert symbol.tick_size == pytest.approx(expected_tick)
    assert symbol.price_increment is None
    assert symbol.cost_override()["tick_size"] == pytest.approx(expected_tick)


@pytest.mark.parametrize("precision_mode", [3, 99])
def test_available_symbols_does_not_invent_tick_for_non_fixed_precision_modes(
    readonly_adapter,
    mock_ccxt_exchange,
    precision_mode,
):
    mock_ccxt_exchange.precisionMode = precision_mode
    mock_ccxt_exchange.load_markets.return_value = {
        "spot": {
            "id": "BTCUSDT",
            "symbol": "BTC/USDT",
...
def readonly_adapter(mock_ccxt_exchange):
    ...
    adapter._decimal_places_precision_mode = 2
...
def test_live_executor_accepts_ccxt_owned_decimal_price_normalization(
    authed_adapter,
    mock_ccxt_exchange,
):
    mock_ccxt_exchange.market.return_value = {
    ...
    prepared = executor.prepare_order(
        OrderRequest(
            client_order_id="crypto-limit-1",
            symbol="BTCUSDT",
            venue_symbol="BTC/USDT",
            side="buy",
            quantity=1.0,
    ...
    assert prepared.limit_price == 100.12


def test_live_executor_rejects_ccxt_change_to_authoritative_price(
    authed_adapter,
    mock_ccxt_exchange,
):
    mock_ccxt_exchange.market.return_value = {
    ...
    mock_ccxt_exchange.amount_to_precision.return_value = "1"
    mock_ccxt_exchange.price_to_precision.return_value = "100.11"
    instrument = SymbolInfo(
        symbol="BTCUSDT",
        market="crypto",
        data_source="binance",
    ...
    with pytest.raises(ValueError, match="authoritative"):
        executor.prepare_order(
            OrderRequest(
                client_order_id="crypto-limit-2",
                symbol="BTCUSDT",
                venue_symbol="BTC/USDT",
                side="buy",
                quantity=1.0,
...
_SWAP = {"symbol": "BTC/USDT:USDT", "type": "swap", "swap": True, "contract": True}
_DELIVERY = {
    "symbol": "BTC/USDT:USDT-260925",
    "type": "future",
    "future": True,
...
_SPOT = {"symbol": "BTC/USDT", "type": "spot", "spot": True}


@pytest.mark.parametrize(
    ("market", "contract_month", "position_effect", "reduce_only"),
    [
        (_SWAP, None, "close", True),
        (_SWAP, None, "reduce", True),
        (_SWAP, None, "open", False),
        (_SWAP, None, "add", False),
        (_DELIVERY, "202609", "close", True),
        (_DELIVERY, "202609", "reduce", True),
        (_DELIVERY, "202609", "open", False),
        (_SPOT, None, "close", False),
        (_SPOT, None, "reduce", False),
    ],
)
def test_place_order_sends_contract_exits_reduce_only(
    authed_adapter, mock_ccxt_exchange, market, contract_month, position_effect, reduce_only
):
    ...
    if contract_month is not None:
        signal["contract_month"] = contract_month
